...
const generateAIContent = async (prompt: string, action: string) => {
    try {
        ...
        const model = genAI.getGenerativeModel({ model: "gemini-1.5-pro-latest", tools: [googleSearch] });
    ...
    try {
        switch (action) {
             case "getReviewSummary":
                ...
                prompt = `As a restaurant operations analyst, summarize customer reviews for \"${locationName}\". First, use the search tool to find recent reviews from the web to supplement the provided review data. Identify key themes, recent positive feedback, and urgent areas for improvement. Use clear headings. Reviews:\n${reviewTexts}`;
                break;

            case "getLocationMarketAnalysis":
                prompt = `Analyze the local market for \"${locationName}\". Be detailed and actionable. Use the search tool to find the most current information on local events, holidays, news, and consumer trends. Include sections for: 1. Recent & Upcoming Local Events & Holidays. 2. Current Macro/Micro Trends. 3. Competitive Landscape. 4. Actionable Opportunities (3-5 specific, creative ideas).`;
                break;
            ...
            case "getSalesForecast":
                ...
                prompt = `As a data analyst, create a 7-day sales forecast for \"${locationName}\". Use the search tool to find any major upcoming local events, holidays, or news that could impact sales. Use this, along with the provided weather forecast and historical data, to create your forecast. Present as a simple, day-by-day list (e.g., "Monday: $XXXX (Sunny, 75°F) - Expected slight increase due to a local festival."). Data: Weather: ${JSON.stringify(weatherForecast)}, History: ${historicalData || "Use general restaurant sales patterns."}`;
                break;

            case "getMarketingIdeas":
                prompt = `Generate creative, actionable, local marketing ideas for the manager of \"${locationName}\". Use the search tool to find current local trends, events, or news to inspire your ideas. Provide ideas for different generations (Gen Z/Millennials, Gen X, Boomers) and budgets (Low/No, Moderate). Structure: For [Generation] ([Focus]): - *Low Budget:* [Idea]. - *Moderate Budget:* [Idea]. Example: For Gen Z (Digital): - Low: \'Host an Instagram contest for best photo in the restaurant.\' - Moderate: \'Partner with a local micro-influencer.\'`;
                break;
            default:
